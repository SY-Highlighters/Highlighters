...
export const useFeedsInBookmark = () => {
  ...
  const getPageBoard = async ({ pageParam = 1 }) => {
    const res = await axios({
      ...
      }/api/bookmark?page=${pageParam}&take=${4}`,
      headers: {
  ...
  const {
    ...
    status: status,
  } = useInfiniteQuery(["feedsBookmark"], getPageBoard, {
  ...
  return { getBoard, getNextPage, getBoardIsSuccess, getNextPageIsPossible, status };
};
